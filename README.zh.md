...
duckling setup       # 扫二维码 / 点链接，TG 里按 Start → 配对完成
duckling start       # daemon 连上 relay
```

然后在 Telegram 上跟 [@DucklingCli_Bot](https://t.me/DucklingCli_Bot) 聊：
...
环境要求：Node 18.17+，本机已经能跑 `claude`（SDK 用你现有的 Claude Code OAuth —— **不用 API key，不会另外扣钱**）。
